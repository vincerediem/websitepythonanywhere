...
def create_open_df(positions, stock_prices, trade_gains_losses, percent_gains_losses):
    open_data = []
    ...
    for stock, data in positions.items():
        ...
        for i in range(len(data['purchase_date'])):
            ...
            trade_gains = current_price * num_shares - purchase_price * num_shares
            percent_gains = (current_price / purchase_price - 1) * 100

            open_data.append([stock, purchase_price, purchase_date, current_price, trade_gains, percent_gains])

    open_df = pd.DataFrame(open_data, columns=['stock', 'purchase_price', 'purchase_date', 'current_price', 'trade_gains', 'percent_gains'])
    return open_df
...
def backtest_strategy(stock_list):
    ...
    open_df = create_open_df(positions, stock_prices, trade_gains_losses, percent_gains_losses)
